...
test("weight updates refresh the chart and list together; settings retain their draft", async ({ page }) => {
  await ready(page);
  await expect(page.getByLabel("체중 (kg)", { exact: true })).toBeEnabled();
  await page.getByLabel("체중 (kg)", { exact: true }).fill("72.4");
  await page.getByRole("button", { name: "수정 저장", exact: true }).click();
  await expect(page.getByRole("status").filter({ hasText: "체중 기록을 수정했습니다." })).toBeVisible();
  await expect(page.locator(".metric-row")).toContainText("72.4");
  await expect(page.locator(".record-list")).toContainText("72.4");
  await nav(page, "설정").click();
  await expect(page.getByLabel("목표 체중 (kg)", { exact: true })).toBeVisible();
  await page.getByLabel("목표 체중 (kg)", { exact: true }).fill("67.5");
  await nav(page, "체중").click();
  await nav(page, "설정").click();
  await expect(page.getByLabel("목표 체중 (kg)", { exact: true })).toHaveValue("67.5");
  ...
  await nav(page, "체중").click();
  await expect(page.locator(".goal-line")).toContainText("67.5");
  await page.reload();
  await expect(page.locator(".metric-row")).toContainText("72.4");
});
